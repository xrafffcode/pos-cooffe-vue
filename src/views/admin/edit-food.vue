...
const fetchFood = async () => {
    const response = await axios.get(`http://localhost:8000/api/foodid/${id}`, {
        headers: {
    ...
    spicy_level.value = response.data.spicy_level
...
const handleSubmit = async () => {
    try {
        ...
        await axios.post(`http://localhost:8000/api/updatefood/${id}`, formData, {
            headers: {
    ...
    <div>
        ...
        <form @submit.prevent="handleSubmit">
            ...
            <div class="mb-3">
                ...
                <input type="file" class="form-control" @change="image = $event.target.files[0]" :class="{ 'is-invalid': errors && errors.image }">
